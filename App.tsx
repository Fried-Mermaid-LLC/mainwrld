...
import { CustomizationView } from './app/CustomizationView';
import {
  View,
  User,
  UserRecord,
  ...
  return (
    <div className={`fixed inset-0 animate-in fade-in duration-500 overflow-hidden flex flex-col ${settings.inverted ? 'bg-black text-white' : 'bg-white text-black'}`}>
      ...
      {settings.scrollMode ? (
          ...
          >
            <div className={`max-w-2xl mx-auto space-y-10 mb-20 reader-content select-none transition-all duration-300 ${isBlurred ? 'blur-xl' : ''}`} style={{ fontSize: `${settings.fontSize}px`, WebkitUserSelect: 'none', userSelect: 'none' }}>
                {!canAccessAll && (
