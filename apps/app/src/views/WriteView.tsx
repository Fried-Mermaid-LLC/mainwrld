...
export const WriteView = () => {
  ...
  const handleBeforeInput = useCallback(
    (event: React.FormEvent<HTMLDivElement>) => {
      ...
      const currentCount = calculateWordCount(
    ...
    async (mode: "manual" | "auto") => {
      ...
      try {
        ...
        if (isSavingNewChapter && currentContent.trim()) {
          ...
          const createdChapterIndex = currentBookId
            ? chapterCountBeforeSave
            : 0;
  ...
  useEffect(() => {
    ...
    if (!selectedBook || selectedChapterIndex === "new") {
      applyContent(
        "",
        selectedBook ? `Chapter ${meta.length + 1}` : "Chapter 1",
      );
      return;
    }
  ...
  // A saved, selected chapter — excludes both the "+ New" draft state and the
  // empty "" state (nothing selected yet), so chapter actions like Delete only
  // appear once a real chapter is open.
  ...
  // Open the Book Details (metadata) editor for an existing book — reached from
  // the pencil button on each cover in the works grid.
  const openBookSettings = (b: Book) => {
    setPublishingInitialData({
      bookId: b.id,
      title: b.title,
      tagline: b.tagline,
    ...
    >
      <div className="w-full flex-1 flex flex-col min-h-0">
        {isWriting ? (
          ...
          // Editor header: back arrow on the left, centered title + book
          // subtitle (matching the Library / Me header style).
          <header className="relative px-6 py-4 border-b border-[#eaeaea] flex items-center justify-center bg-white z-50">
            <button
              onClick={onBack}
          ...
          <header className="px-6 py-4 border-b border-[#eaeaea] flex items-center justify-center">
            <div className="flex flex-col items-center gap-1">
              ...
              <p className="text-[13px] font-semibold text-[#9aa1a9] tracking-[0.13px] leading-[1.2]">
                {myWorks.length} {myWorks.length === 1 ? "Book" : "Books"}
              </p>
            </div>
          </header>
        )}
            ...
            >
              {!isWriting && (
                <div className="space-y-4">
                  {/* Book title is edited on the Book Details screen, not here. */}
                ...
                is a clearly separate dashed action rather than a hidden option. */}
                    <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-1 px-1 py-1">
                      {(selectedBook?.chapterMeta ?? []).map(
                        (ch: any, idx: number) => {
                          const active =
                            selectedChapterIndex === idx.toString();
                          ...
                          return (
                            <button
                              ...
                              onClick={() =>
                                setSelectedChapterIndex(idx.toString())
                              }
                              title={ch.title}
